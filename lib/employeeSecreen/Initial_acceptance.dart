  ...
  delete_company(var comp_id)async {
    ...
    await user
      ...
      if(value.docs.isNotEmpty){
        ...
        {
          num_companies_follow = value.docs[k]
              .data()[
          'companies_follow']
              .length;
          ...
          i++) {
            ...
            [i] ==
              ...
              val.add(
                  '${value.docs[k].data()['companies_follow'][i]}');
              user
                  .doc(value.docs[k].id)
                  .update({
                "companies_follow":
    ...
    {
      await user.doc(user_Id[k]).collection('companies_saved').get().then((value) async {
        if (value.docs.isNotEmpty) {
          for (int j = 0; j < value.docs.length; j++) {
            if (value.docs[j].data()['company_Id'] ==
                comp_id) {
              user.doc(user_Id[k]).collection('companies_saved')
                  .where("company_Id",
                  isEqualTo:comp_id)
                  .get()
                  .then((value) {
